...
def get_comps_by_industry(industry, exclude_ticker=None):
  url = f"{FMP_URL}/stock-screener?industry={industry}&limit=10&apikey={FMP_API_KEY}"
  data = get_json(url)
  comps = [c for c in data if c["symbol"].lower() != exclude_ticker.lower()]
  return comps
...
def build_valuation_model(ticker):
  ...
  comps = get_comps_by_industry(industry, ticker)

  peer_data = []
  for comp in comps:
    try:
      r = get_ratios(comp["symbol"])
      peer_data.append({
        "ticker": comp["symbol"],
  ...
  # Convert EV to equity if needed
...
def median_or_average(data, key):
  values = [d[key] for d in data if d[key] > 0]
  if not values:
    return 1
  values.sort()
  n = len(values)
